...
- **agent 路径**：经 `CottageAgent.runAssistantTurn` 工具循环，具备完整治理——连续失败循环、PlanToolGuard、policyGate 审批、trace、输出后处理。
...
- 工具循环：`src/agent/CottageAgent.ts`。顺序：连续失败 doomLoop check（软提醒→policyGate）→ PlanToolGuard（Plan Mode）→ policyGate（异步）→ 执行（askUser 特殊路径/流式/invoke）→ strip 输出后处理 → doomLoop record → trace → ToolMessage。
...
- `PlanToolGuard` 只在公开 Plan Mode 生效，按已批准 revision、当前步骤、路径范围与预算判定；状态与计数来自 `src/plan/` 的 PlanRunner/PlanRepository。
...
| PlanToolGuard | Plan Mode 启用 | 继承当前 Plan Mode 范围 | 关 |
...
| `src/agent/CottageAgent.ts` | 加 getter：`getPolicyGate()`/`getPlanToolGuard()`/`getDoomLoopDetector()`/`getToolExecutor()`（构造参数收 executor） |
...
| `src/plan/scopeGate.ts` | 统一约束 Agent 与脚本来源的 Plan Mode 工具调用 |
...
- **3a 闸门段收编**：把循环内判定（doom check→PlanToolGuard→policy，约 L1623-1757）替换为 executor 的 `evaluateGates(req, hooks)` 纯判定入口；hooks 注入 `onAwaitingApproval`（→setCallInteraction）与 `onPreRiskyCheckpoint`；`policyApproval:'gate'` 在此启用。逐行等价替换，不改行为。
- **3b 执行段收编**：「执行+strip 输出+doom record+trace append」替换为 `executor.invoke`，executor 加 `onStreamDelta` hook 承接流式（callSection.result 累积）；**askUser 特殊路径保留在循环内**（交互不是工具执行）；`finalizeInterruptedToolCalls`/checkpoint/staging 逻辑不动。
- 每步跑现有全部测试 + 手工回归清单（见验证）。

## 验证

- 单测（`toolInvocation.test.ts`）：script 黑名单拒绝/deny 拦截/confirm 放行（统一审批语义）/PlanToolGuard 范围阻断/相同调用可重复执行/doom record 被调/trace 收到 source:'script'/signal abort；manual confirm 放行+trace。
...
- 手工回归清单：
  ...
  4. 阶段 3 后：审批允许/拒绝、doom loop 触发、Plan Mode 范围拦截、runScript 流式 logs、写文件 diff 卡片、中途停止、askUser 全路径行为与改前一致。
...
3. **Plan Mode 预算**：批量脚本中的每次外部调用与实际文件变更都必须计入批准预算，不能只统计宿主 runScript。
